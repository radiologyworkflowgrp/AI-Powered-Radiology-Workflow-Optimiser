            ...
            <Card key={prescription.id}>
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div>
                    <CardTitle>{prescription.medication || prescription.medicineName}</CardTitle>
                    <CardDescription>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid grid-cols-3 gap-4 pt-2">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{prescription.dosage || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium">Frequency</p>
                    <p className="text-sm text-muted-foreground">{prescription.frequency || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium">Duration</p>
                    <p className="text-sm text-muted-foreground">{prescription.duration || 'N/A'}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between pt-4 border-t">
                  ...
                  {prescription.status === "approved" && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => handleRequestRefill(prescription.id || prescription._id)}
                    >
                      Request Refill
                    </Button>
                  )}
