                    ...
                    <CardContent>
                        {loading ? (
                            ...
                            <div className="space-y-3">
                                {logs.map((log) => (
                                    <div key={log.id || log._id} className="border-b pb-3 last:border-0">
                                        <div className="flex items-start justify-between">
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <Badge variant={(log.logType || log.type) === 'login' ? 'default' : 'secondary'}>
                                                ...
                                                <p className="text-sm font-medium">{log.description || log.message || `${log.action}`}</p>
                                                {log.email && (
                                            ...
                                            <span className="text-xs text-muted-foreground whitespace-nowrap ml-4">
                                                {new Date(log.created_at || log.timestamp || log.createdAt).toLocaleString()}
                                            </span>
                                        </div>
                                    </div>
                                ))}
