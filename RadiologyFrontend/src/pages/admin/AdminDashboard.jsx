...
const AdminDashboard = () => {
  ...
  const getAvailabilityVariant = (availability) => {
    ...
    if (status === 'available') return 'default'; // Green
    if (status === 'busy') return 'destructive'; // Red
    if (status === 'unavailable') return 'secondary'; // Grey
    if (status === 'on-call') return 'secondary'; // Blue
  ...
  const getAvailabilityColor = (availability) => {
    ...
    if (status === 'busy') return 'bg-red-500 text-white hover:bg-red-600';
    if (status === 'unavailable') return 'bg-gray-400 text-black hover:bg-gray-500';
    if (status === 'on-call') return 'bg-blue-500 text-white hover:bg-blue-600';
    return '';
  };
            ...
            <div className="rounded-md border">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {logs.length > 0 ? (
                    logs.map((log) => (
                      <tr key={log.id || Math.random()} className="border-t hover:bg-muted/50">
                        <td className="p-2 text-muted-foreground whitespace-nowrap">
                          {new Date(log.created_at || log.createdAt || log.loginTime).toLocaleString()}
                        </td>
                        <td className="p-2 font-medium">{log.action || 'Login'}</td>
                        <td className="p-2">{log.description || `User ${log.email} logged in`}</td>
                        <td className="p-2">
                          <Badge variant="outline" className="capitalize">
                            {log.type || log.entityType || 'System'}
                          </Badge>
                        </td>
                        <td className="p-2 font-mono text-xs">{log.entityId || log.userId || '-'}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
