...
// Explicitly register wadouri manager
cornerstoneWADOImageLoader.wadouri.register(cornerstone);

// Configure Web Workers
cornerstoneWADOImageLoader.webWorkerManager.initialize({
    maxWebWorkers: navigator.hardwareConcurrency || 1,
    startWebWorkersOnDemand: false, // Start immediately to catch errors early
    taskConfiguration: {
        decodeTask: {
            initializeCodecsOnStartup: true, // Force codec loading
            usePDFJS: false,
            strict: false,
        },
    },
    ...
    useEffect(() => {
        ...
        const loadDicomImage = async () => {
            try {
                setLoading(true);
                setError(null);
                console.log("Attempting to load DICOM for ScanID:", scanId);

                // Enable the cornerstone element
                const element = viewerRef.current;
                cornerstone.enable(element);

                // 1. Fetch the binary data manually
                const baseUrl = import.meta.env.VITE_API_URL || 'http://localhost:3002';
                const fetchUrl = `${baseUrl}/api/dicom/${scanId}/file`;
                ...
                // 2. Parse and Validate using dicom-parser
                let dataSet;
                try {
                    const byteArray = new Uint8Array(arrayBuffer);
                    dataSet = dicomParser.parseDicom(byteArray);

                    // Check for SOPClassUID (0008,0016)
                    const sopClassUid = dataSet.string('x00080016');
                    if (!sopClassUid) {
                        throw new Error("Missing SOPClassUID. This is not a valid DICOM image.");
                    }
                    console.log("Valid DICOM. SOPClassUID:", sopClassUid);
                } catch (parseErr) {
                    console.error("DICOM Parsing Error:", parseErr);
                    throw new Error(`File corrupted or invalid DICOM: ${parseErr.message}`);
                }

                // 3. Create generic Blob for Cornerstone
                const blob = new Blob([arrayBuffer], { type: 'application/dicom' });
                const imageId = cornerstoneWADOImageLoader.wadouri.fileManager.add(blob);
                console.log("Generated local ImageId:", imageId);

                // =========================================================
                // CUSTOM METADATA PROVIDER
                // We manually provide metadata to bypass library bugs with Secondary Capture images
                // =========================================================
                const customProvider = (type, queryImageId) => {
                    if (queryImageId !== imageId) return;

                    if (type === 'imagePlaneModule') {
                        // Return valid defaults for missing tags to prevent crash
                        return {
                            ...
                            sliceThickness: 1,
                            sliceLocation: 0,
                            frameOfReferenceUID: '1.2.3.4.5.6.7', // Dummy UID
                    ...
                    if (type === 'generalSeriesModule') {
                        return {
                            ...
                            seriesNumber: dataSet.float('x00200011'),
                        };
                    }
                ...
                // Add provider with high priority
                cornerstone.metaData.addProvider(customProvider, 10000);
                ...
                try {
                    ...
                    // Get viewport to extract metadata for UI
                    ...
                    // Extract basic metadata for UI display
                    if (image.data) {
                        const imageMetadata = {
                            rows: image.rows,
                    ...
                    // Cleanup provider after load attempt (success or fail) to avoid polluting global state
                    cornerstone.metaData.removeProvider(customProvider);
                }
            } catch (err) {
        ...
        // Handle Window Resize (for browser zoom/resize sync)
        const handleResize = () => {
        ...
        return () => {
            ...
            // Note: We should technically remove the blob from fileManager, but not critical for simple viewer
        };
    }, [scanId]);
            ...
            <div className="w-full h-full max-w-7xl mx-auto p-4 flex flex-col">
                ...
                <div className="flex-1 flex gap-4">
                    ...
                    <div className="flex-1 bg-black rounded-lg overflow-hidden relative">
                        {loading && (
                            <div className="absolute inset-0 flex items-center justify-center text-white">
                                <div className="text-center">
                                    ...
                                    <p className="text-xs text-gray-400 mt-2">Connecting to worker...</p>
                                </div>
                            </div>
                        )}
                        ...
                        {error && (
                            <div className="absolute inset-0 flex items-center justify-center text-white">
                                <div className="text-center bg-red-900 bg-opacity-50 p-8 rounded-lg max-w-lg">
