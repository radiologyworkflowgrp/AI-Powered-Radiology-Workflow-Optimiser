...
def log_to_backend(action: str, description: str, entity_type: str = "system", entity_id: str = "ML_WORKER", metadata: dict = None):
    """
    Send log entry to backend API to act as centralized logging.
    """
    try:
        ...
        # Don't crash worker if logging fails
...
def run_worker(model_dir: str = str(DEFAULT_MODEL_DIR)):
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            case_id = payload.get("case_id") or payload.get("patient_id")
            ...
            try:
                ...
                if GLOBAL_MODEL is not None and GLOBAL_VOCAB is not None and GLOBAL_CONFIG is not None:
                    try:
                        ...
                        # Centralized Logging
                        log_to_backend(
                            action="ML_PREDICTION_SUCCESS",
                            description=f"{patient_name} | ML prediction success (Priority: {final_priority})",
                            entity_type="ml_job",
                            entity_id=str(case_id or "unknown"),
                            metadata={"patientName": patient_name, "status": "ML prediction success", "priority": final_priority, "duration": time.time()-start}
                        )
                        ...
                        log_to_backend(
                            action="ML_PREDICTION_FALLBACK",
                            description=f"{patient_name} | ML fallback to rules",
                            entity_type="ml_job",
                            entity_id=str(case_id or "unknown"),
                            metadata={"patientName": patient_name, "status": "ML fallback to rules", "error": str(e)}
                        )
                    ...
                    log_to_backend(
                        action="ML_RULE_BASED",
                        description=f"{patient_name} | ML rule-based priority",
                        entity_type="ml_job",
                        entity_id=str(case_id or "unknown"),
                        metadata={"patientName": patient_name, "status": "ML rule-based priority"}
                    )
                ...
                log_to_backend(
                    ...
                    description=f"{patient_name} | ML processing failed",
                    entity_type="ml_job",
                    entity_id=str(case_id or "unknown"),
                    metadata={"patientName": patient_name, "status": "ML processing failed", "error": str(e)}
                )
        except Exception as e:
