...
class DuoFormerInference:
    ...
    def preprocess_image(self, image_bytes: bytes) -> torch.Tensor:
        ...
        """
        try:
            # Try to detect if this is a DICOM file
            is_dicom = image_bytes[:4] == b'DICM' or image_bytes[128:132] == b'DICM'
            ...
            if is_dicom:
                ...
                logger.info("Detected DICOM file, converting to image...")
                dcm = pydicom.dcmread(IOBytes(image_bytes))
                ...
                logger.info(f"DICOM converted to image: {image.size}, mode: {image.mode}")
...
def main():
    ...
    if args.worker:
        worker = XRayWorker(args.model, args.ground_truth_csv)
        worker.start()
    else:
        parser.print_help()
