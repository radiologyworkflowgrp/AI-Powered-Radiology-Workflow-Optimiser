...
const fs = require('fs');
const { DICOMImage, Patient, ActivityLog } = require('../models');
...
router.post('/upload', upload.single('dicomFile'), async (req, res) => {
    try {
        ...
        console.log('✓ DICOM image metadata saved to PostgreSQL');

        // Log Activity: DICOM Uploaded
        try {
            await ActivityLog.create({
                action: 'dicom_uploaded',
                description: `${patient.name} | Patient DICOM upload success`,
                entityType: 'dicom',
                entityId: dicomImage.id.toString(),
                metadata: {
                    patientId: patientId,
                    patientName: patient.name,
                    status: 'Patient DICOM upload success',
                    filename: req.file.originalname
                }
            });
        } catch (logError) {
            console.error('Error logging DICOM upload:', logError);
        }
        ...
        try {
            ...
            try {
                ...
                if (token) {
                    ...
                    doctorId = decoded.referenceId; // PostgreSQL doctor ID
                    console.log(`✓ Extracted doctor_id from token: ${doctorId}`);
                }
            } catch (tokenError) {
                console.warn('⚠️ Could not extract doctor_id from token:', tokenError.message);
            }
            ...
            const queueData = {
                ...
                doctor_id: doctorId, // Track uploading doctor
 ...
 * Retrieve DICOM image binary data by Scan ID (e.g. DICOM_12345) OR UUID
 * This endpoint is specifically for the Dicom Viewer
 */
router.get('/:scanId/file', async (req, res) => {
    try {
        const { scanId } = req.params;
        logger.info(`Request for DICOM file with Scan ID: ${scanId}`);

        // Try finding by scan_id string first
        let dicomImage = await DICOMImage.findOne({ where: { scan_id: scanId } });

        // If not found, try by Primary Key (UUID/Int) if it looks like one
        if (!dicomImage && (scanId.length === 36 || !isNaN(scanId))) {
            dicomImage = await DICOMImage.findByPk(scanId);
        }

        if (!dicomImage) {
            logger.warn(`DICOM not found for ID: ${scanId}`);
            return res.status(404).json({
                success: false,
        ...
        logger.info(`Serving DICOM file: ${dicomImage.scan_id}, Size: ${dicomImage.image_data?.length} bytes`);
...
/**
 * GET /api/dicom/:id
 * Retrieve DICOM image binary data (Legacy/Direct ID access)
 */
