...
const { MLReport, Patient, ActivityLog } = require('../models');
...
router.post('/',
  ...
  async (req, res) => {
    try {
      ...
      // Update patient status to 'Doctor Appointment' when report is generated
      try {
        const patient = await Patient.findByPk(req.body.patient_id);
        if (patient && patient.status === 'Admitted') {
          await patient.update({ status: 'Doctor Appointment' });
          console.log(`Updated patient ${patient.id} status to 'Doctor Appointment'`);
        }
      } catch (patientError) {
        console.error('Error updating patient status:', patientError);
        // Don't fail the report creation if patient update fails
      }
      ...
      // Log Update: ML Report Generated
      // Action: report_generated
      try {
        await ActivityLog.create({
          action: 'report_generated',
          description: `${req.body.patient_name} | Patient report generated success`,
          entityType: 'report',
          entityId: report.id.toString(),
          metadata: {
            patientName: req.body.patient_name,
            status: 'Patient report generated success',
            reportType: 'ML',
            confidence: req.body.confidence_score,
            model: req.body.ml_model
          }
        });
      } catch (logError) {
        console.error('Error logging ML report:', logError);
      }
    } catch (error) {
